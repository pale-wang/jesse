...
async def test_insert(loop):
    await create_pool(loop=loop, user='root', password='root', db='awesome')
    rs = await execute('insert into users(id, email, passwd, admin, name, image, created_at) values (?, ?, ?, ?, ?, ?, ?)', ('11111', 'orm_new@example.com', '1234567890', 0, 'orm_new', 'blank:orm_new', 0.0))
    print(rs)
